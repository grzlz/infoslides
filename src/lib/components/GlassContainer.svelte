<script>
	/**
	 * GlassContainer - Liquid glass morphism effect
	 * Perfect for dialogue bubbles, cards, overlays
	 */
...
<div
	class="GlassContainer {className}"
	style:--corner-radius={cornerRadius}
	style:--base-strength={baseStrength}
	style:--extra-blur={extraBlur}
	style:--softness={softness}
	style:--tint-amount={tintAmount}
	style:--tint-saturation={tintSaturation}
	style:--tint-hue={tintHue}
...
<style>
	...
	.GlassMaterial .GlassEmbossReflection {
		backdrop-filter: blur(calc(var(--total-strength) * 1.5)) invert(0.25) brightness(1.11)
			saturate(1.2) hue-rotate(-10deg) contrast(2.3);
